...
class TestThemeApplicationFromConfig:
    ...
    def test_list_command_applies_theme_from_config(self, tmp_path):
        ...
        # Patch get_claude_paths to return empty so the command exits fast after theme is applied
        with patch("par_cc_usage.main.apply_temporary_theme") as mock_apply, patch(
            "par_cc_usage.config.Config.get_claude_paths", return_value=[]
        ):
